...
import com.example.app_quickquiz.model.UserAnswer;
...
public class QuizActivity extends AppCompatActivity {
    ...
    private List<UserAnswer> uanswerList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        quizRId = getIntent().getStringExtra("quiz_result_id");
...
//        }
    ...
    private String getUserSelectedAnswerId(String questionId) {
        for (UserAnswer ua : uanswerList) {
            if (ua.getQuestion_id().equals(questionId)) {
                return ua.getAnswer_id();
            }
        }
        return null;
    }
    ...
    private void displayAnswers() {
        ...
        String userSelectedAnswerId = getUserSelectedAnswerId(currentQuestion.getId());

        for (int i = 0; i < buttons.length; i++) {
            buttons[i].setVisibility(View.VISIBLE);
            if (i < answerList.size()) {
                Answer answer = answerList.get(i);
                buttons[i].setText(answer.getAnswer());

                boolean isCorrect = answer.getAnswer().equals(answer.isCorrect());
                boolean isUserSelected = answer.getAnswer().equals(userSelectedAnswerId);

                // Thiết lập kiểu chữ
                if (isCorrect && isUserSelected) {
                    buttons[i].setTypeface(null, Typeface.BOLD_ITALIC); // Đúng và đã chọn
                } else if (isCorrect) {
                    buttons[i].setTypeface(null, Typeface.BOLD); // Đúng
                } else if (isUserSelected) {
                    buttons[i].setTypeface(null, Typeface.ITALIC); // Người dùng đã chọn
                } else {
                    buttons[i].setTypeface(null, Typeface.NORMAL); // Sai
                }
